...
class RStrainDataset(Dataset):
    def __init__(self, root=None,img_transforms = img_transforms, mask_transforms = mask_transforms):
        ...
        #key_word1 = '(2)'
        key_word1 = 'label'
        cityname = ["aachen", "bielefeld","dortmund","duesseldorf","koeln","muenster"]
        #cityname = ["aachen","abancay","accra","al_qurnah","austin"]
        for i in cityname:
          roots = root + "/" + i + '/labels'
          for dirname in os.listdir(roots):
            '''
            if not key_word1 in dirname:
             continue
            '''
            #print(dirname)
            dirname = os.path.join(roots, dirname)
            #print(dirname)
            img_dir = dirname.replace("labels","images"),dirname
...
if __name__ ==  "__main__":
  train_pic_root = "/content/drive/My Drive/Zurich_dataset_v1.0/images_tif"
